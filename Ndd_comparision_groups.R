...
NDDx_comparision_Rawdata <- read_excel ("DataExtraction_NDDx_Pairs.xlsx")
...
# writexl::write_xlsx(ADHD_ASD_group,"ADHD_ASD.xlsx",col_names = TRUE)
# writexl::write_xlsx(ADHD_SLD_group,"ADHD_SLD.xlsx",col_names = TRUE)
# writexl::write_xlsx(ADHD_CTD_group,"ADHD_CTD.xlsx",col_names = TRUE)
# writexl::write_xlsx(ADHD_DCD_group,"ADHD_DCD.xlsx",col_names = TRUE)
# writexl::write_xlsx(ASD_SLD_group,"ASD_SLD.xlsx",col_names = TRUE)
# writexl::write_xlsx(DS_WS_group,"DS_WS.xlsx",col_names = TRUE)
...
for (domain_label in unique(ADHD_ASD_group$EF.Domain)){
  ...
  if (nrow(row_basedon_ef_domain)>=3){
    ...
    if (pooled_effect_p < 0.001) {
      ...
      result <- sprintf("%.3f", pooled_effect_p)
    }
...
## combining all comparison group results per EF domain into one
### plotting figure 4
combined_EF_perfomain <- bind_rows(ADHD_ASD_efdomains_dataframe, ADHD_SLD_efdomains_dataframe, ADHD_DCD_efdomains_dataframe,ADHD_CTD_efdomains_dataframe,ASD_SLD_efdomains_dataframe,DS_WS_efdomains_dataframe)
combined_EF_perfomain<-subset(combined_EF_perfomain, N>2)
combined_EF_perfomain <- combined_EF_perfomain %>%mutate(Index = row_number())
combined_EF_perfomain <- combined_EF_perfomain %>%
...
grps <- unique(combined_EF_perfomain$comparison) # create list of groups from data frame

# generate all plots; for the top plot we enable data table labels:
g1 <- fplot(combined_EF_perfomain, grps[1], var_names = TRUE)
g2 <- fplot(combined_EF_perfomain, grps[2], var_names = FALSE)
g3 <- fplot(combined_EF_perfomain, grps[3], var_names = FALSE)
g4 <- fplot(combined_EF_perfomain, grps[4], var_names = FALSE)
# for the final plot we enable x-axis labels:
g5 <- fplot(combined_EF_perfomain, grps[5], x_axis = TRUE, var_names = FALSE)
...
Finalplot4 <- ggdraw(finalplot) +
  ...
  draw_text(
    "Executive Functioning Across Different NDC Groups", 
  ...
  draw_plot_label(x = 0.205,y = .972, label = "Favours ADHD   Favours ASD", 
                  hjust = 0, size = 7,family="Helvetica",fontface = "bold") +
  draw_plot_label(x = 0.205,y = .7, label = "Favours ADHD   Favours SLD", 
                  hjust = 0, size = 7,family="Helvetica",fontface = "bold")+
  draw_plot_label(x = 0.205,y = .42, label = "Favours ADHD   Favours TD", 
                  hjust = 0, size = 7,family="Helvetica",fontface = "bold")+
  draw_plot_label(x = 0.21,y = .23, label = "Favours ASD   Favours SLD", 
                  hjust = 0, size = 7,family="Helvetica",fontface = "bold")+
  draw_plot_label(x = 0.215,y = .16, label = "Favours DS   Favours WS", 
                  hjust = 0, size = 7,family="Helvetica",fontface = "bold")
ggsave(
  "Figure4_Aug2024_nesting.eps",
  plot = `Finalplot4`,
  path = NULL,
  scale = 1,
  width = 180,
...
## plot NDDx comparision groups performance only studies
### plotting Supplementary Figure 2
combined_EF_perfomain_perfonly <- bind_rows(ADHD_ASD_perfonly_efdomains_dataframe, ADHD_DCD_perfonly_efdomains_dataframe,ADHD_CTD_perfonly_efdomains_dataframe,ADHD_SLD_efdomains_dataframe,DS_WS_perfonly_efdomains_dataframe)
combined_EF_perfomain_perfonly<-subset(combined_EF_perfomain_perfonly, N>2)
combined_EF_perfomain_perfonly <- combined_EF_perfomain_perfonly %>%mutate(Index = row_number())

grps <- unique(combined_EF_perfomain_perfonly$comparison) # create list of groups from data frame

# generate all plots; for the top plot we enable data table labels:
g1 <- fplot(combined_EF_perfomain_perfonly, grps[1], var_names = TRUE)
g2 <- fplot(combined_EF_perfomain_perfonly, grps[2], var_names = FALSE)
g3 <- fplot(combined_EF_perfomain_perfonly, grps[3], var_names = FALSE)
# for the final plot we enable x-axis labels:
g4 <- fplot(combined_EF_perfomain_perfonly, grps[4], x_axis = TRUE, var_names = FALSE)
# g5 <- fplot(combined_EF_perfomain_perfonly, grps[5], x_axis = TRUE, var_names = FALSE)
...
Finalplot5 <- ggdraw(finalplot) +
  draw_plot(finalplot, 0, 0, 1, 1) +
  draw_text(
    "NDC Group Comparison (Performance Only)", 
    x = 0.5, y = 1,  # Center horizontally and place at the top
    hjust = 0.5, vjust = 1.5,  # Adjust vertical position to move title up or down
    size = 7, fontface = "bold",family="Helvetica"
  )+
  #  Code here creates a title at the centre of the forest plot
  draw_plot_label(x = 0.312,y = .96, label = "Favours ADHD   Favours ASD", 
                  hjust = 0, size = 7,family="Helvetica",fontface="bold") +
  draw_plot_label(x = 0.312,y = .66, label = "Favours ADHD   Favours TD", 
                  hjust = 0, size = 7,family="Helvetica",fontface="bold")+
  draw_plot_label(x = 0.312,y = .38, label = "Favours ADHD   Favours SLD", 
                  hjust = 0, size = 7,family="Helvetica",fontface="bold")+
  # draw_plot_label(x = 0.205,y = .2, label = "Favours ASD   Favours SLD", 
  #                 hjust = 0, size = 7)+
  draw_plot_label(x = 0.33,y = .15, label = "Favours DS   Favours WS", 
                  hjust = 0, size = 7,family="Helvetica",fontface="bold")
ggsave(
  "SupplementaryFigure2_Aug2024_nesting.eps",
  plot = `Finalplot5`,
  path = NULL,
  scale = 1,
  width = 180,
...
Finalplot6 <- ggdraw(finalplot) +
  draw_plot(finalplot, 0, 0, 1, 1) +
  draw_text(
    "NDC Group Comparison (Informant Only)", 
    x = 0.5, y = 1,  # Center horizontally and place at the top
    hjust = 0.5, vjust = 1.5,  # Adjust vertical position to move title up or down
    size = 7, fontface = "bold",family="Arial"
  )+
  #  Code here creates a title at the centre of the forest plot
  draw_plot_label(x = 0.247,y = 0.97, label = "Favours ADHD   Favours ASD", 
                  hjust = 0, size = 7,family="Arial",fontface="bold") +
  # draw_plot_label(x = 0.3,y = .66, label = "Favours ADHD   Favours TD", 
  #                 hjust = 0, size = 10)+
  draw_plot_label(x = 0.247,y = .58, label = "Favours ADHD   Favours SLD", 
                  hjust = 0, size = 7,family="Arial",fontface="bold")
# draw_plot_label(x = 0.205,y = .2, label = "Favours ASD   Favours SLD",
#                 hjust = 0, size = 10)+
...
#                 hjust = 0, size = 10)
ggsave(
  "SupplementaryFigure3_Aug2024_nesting.eps",
  plot = `Finalplot6`,
  path = NULL,
  scale = 1,
  width = 180,
